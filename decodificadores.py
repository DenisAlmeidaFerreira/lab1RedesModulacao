...
def decode_nrz(audio_signal, num_bits, sample_rate=SAMPLE_RATE,debug=False):
    ...
    fn.show("Decodificando NRZ:",debug)
    ...
    for i in range(num_bits):
        ...
        if end_idx > len(audio_signal):
            fn.show(f"Aviso: Áudio muito curto para {num_bits} bits",debug)
            break
        ...
        fn.show(f"Bit {i}: freq={freq:.1f}Hz -> '{bit}'",debug)
...
def decode_manchester(audio_signal, num_bits, sample_rate=SAMPLE_RATE,debug=False):
    ...
    fn.show("Decodificando Manchester:",debug)
    ...
    for i in range(num_bits):
        ...
        if state1 == '1' and state2 == '0': # Alto -> Baixo
            bit = '1'  
            fn.show(f"Bit {i}: {freq1:.1f}Hz -> {freq2:.1f}Hz = alto->baixo = '1'",debug)
        elif state1 == '0' and state2 == '1': # Baixo -> Alto
            bit = '0'  
            fn.show(f"Bit {i}: {freq1:.1f}Hz -> {freq2:.1f}Hz = baixo->alto = '0'",debug)
        else: # Erro de decodificação
            bit = '?'  
            fn.show(f"Bit {i}: {freq1:.1f}Hz -> {freq2:.1f}Hz = ERRO na transição",debug)
